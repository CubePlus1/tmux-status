...
- `process_instances` maps each signed 32-bit agent PID to exactly one `<pid>:`-prefixed start identity, and one process incarnation cannot be reused by multiple conversations; it is never conversation-ID evidence.
