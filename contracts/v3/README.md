...
- A confirmed conversation requires process-associated file or CLI evidence, a UUID, `stable_mapping_key` derived exactly as `<tool>:<conversation_id>`, and `resume_command`.
...
- `process_instances` maps each agent PID to exactly one start identity so PID reuse cannot merge unknown observations; it is never conversation-ID evidence.
